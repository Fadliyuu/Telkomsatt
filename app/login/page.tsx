...
function LoginContent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nextPath = searchParams.get("next");
      const targetPath =
        getAccessiblePath(userData.role, nextPath) ||
        getDefaultPath(userData.role);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#0a0c0f] text-white selection:bg-telkomsat-red selection:text-white font-sans antialiased flex items-center justify-center p-4">
      ...
      <div className="relative z-10 my-8 grid w-full max-w-6xl items-center gap-10 lg:grid-cols-12">
        <aside
          className={`hidden transition-all duration-700 lg:col-span-7 lg:block ${mounted ? "translate-x-0 opacity-100" : "-translate-x-5 opacity-0"}`}
        >
          <div className="max-w-xl">
            ...
            <h2 className="text-4xl font-black leading-[1.12] tracking-tight xl:text-5xl">
              ...
              <span className="mt-2 block bg-gradient-to-r from-[#f42f28] via-red-400 to-orange-400 bg-clip-text text-transparent">
                dalam satu portal.
              </span>
            </h2>
            <p className="mt-6 max-w-lg text-base leading-relaxed text-gray-400">
              Pantau unit sparepart, proses pengajuan, dan verifikasi
              perpindahan barang secara akurat untuk operasional Telkomsat
              Regional 6.
            </p>
            <div className="mt-9 grid grid-cols-3 gap-3">
              {[
                ...
                },
                {
                  icon: ClipboardCheck,
                  label: "Verifikasi",
                  text: "Alur terkontrol",
                },
              ].map((item) => (
                <div
                  key={item.label}
                  className="group rounded-2xl border border-white/10 bg-white/[0.045] p-4 backdrop-blur-xl transition-all hover:-translate-y-1 hover:border-telkomsat-red/30 hover:bg-white/[0.07]"
                >
                  <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl border border-telkomsat-red/20 bg-telkomsat-red/10 text-telkomsat-red transition-colors group-hover:bg-telkomsat-red group-hover:text-white">
            ...
            <div className="mt-8 flex items-center gap-4 border-t border-white/10 pt-6">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500">
                  Admin Sistem · Admin Gudang · Teknisi · Supervisor
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="w-full max-w-md justify-self-center lg:col-span-5 lg:max-w-none">
          {/* Main Glass Card */}
          <div className="relative space-y-8 overflow-hidden rounded-3xl border border-white/15 bg-white/10 p-8 shadow-2xl backdrop-blur-2xl animate-fade-in lg:p-10">
            ...
            >
              {resetMode && resetSent && (
                <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-xs text-emerald-300 flex items-start gap-3">
                  ...
                  <p>
                    Link pemulihan password telah dikirim ke email Anda. Periksa
                    folder Inbox atau Spam.
                  </p>
                </div>
              )}

              {/* Username / reset email field */}
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    id={resetMode ? "reset-email" : "username"}
                    type={resetMode ? "email" : "text"}
                    autoCapitalize="none"
                    value={resetMode ? resetEmail : username}
                    onChange={(e) => {
                      if (resetMode) setResetEmail(e.target.value);
              ...
              >
                {loading || resetLoading ? (
                  <>
                    ...
                    <span>
                      {resetMode ? "Mengirim link..." : "Memproses..."}
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span>
                      {resetMode ? "Kirim Link Pemulihan" : "Masuk ke Sistem"}
                    </span>
                  </>
                )}
              </button>
            ...
            <div className="pt-4 border-t border-white/10 text-center space-y-3 text-xs text-gray-400">
              ...
              <div>
                <Link
                  href="/"
                  className="text-telkomsat-red hover:underline font-bold"
                >
                  ← Kembali ke Beranda
                </Link>
              </div>
          ...
          <p className="text-center mt-6 text-xs text-gray-400">
            © {new Date().getFullYear()} Telkomsat Regional 6. Hak Cipta
            Dilindungi.
          </p>
        </div>
      </div>
